...
public class Student extends Employee implements Observer {
    ...
    public void receiveOffer(String nameCompany, int salary, Vacancy vacancy) {
        if (this.vacancy.equals(vacancy)) {
            ...
            System.out.printf("Студент %s: Мне не подходит данная вакансия!(компания: %s; заработная плата %d; вакансия %s)\n",
                    name, nameCompany, salary, vacancy.getDescription());
        }
    }
    ...
    public String toString() {
        return "Студент " + name + " ищет вакансию " + vacancy.getDescription() + "\n";
    }
}
